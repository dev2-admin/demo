        ...
        setTimeout(function() {
            $.ajax({
                ...
                error: function(res) {
                    ...
                    hideProgress();
                    KTUtil.addClass(target, resultClass);
                    KTUtil.setHTML(resultWrapper, '<span class="kt-quick-search__message">Connection error. Please try again later.</span>');
                    showDropdown();
                    KTUtil.scrollUpdate(resultWrapper);
                }
            });
	...
	var initAsideToggler = function() {
		...
		asideToggler.on('toggle', function(toggle) {
			KTUtil.addClass(body, 'kt-aside--minimizing');
             ...
             //assets/media/logos/logo-9-sm.png
            //var url = @Url.Content("~/Content/assets/media/logos/logo-white.png");

            if (toggler == 0) {
                var imgUrl = $('input[name=dynamicImageUrl]').val();
                
                ssrrcc = $('.kt-aside__brand-logo img').attr('src');
                 $('.kt-aside__brand-logo img').css('width', '50').attr('src', imgUrl);
                toggler = 1
             }else{
                $('.kt-aside__brand-logo img').attr('src', ssrrcc);
                setTimeout(function(){
